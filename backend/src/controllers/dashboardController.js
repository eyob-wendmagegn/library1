...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    if (user.role === 'admin') {
      ...
      const totalReports = await db.collection('reports')?.countDocuments() || 0;
      ...
      const growthData = months.map((_, i) => {
        ...
        return userGrowth.find(g => g._id === monthStr)?.count || 0;
      });
      ...
      const activityData = Array(4).fill(0).map((_, i) => {
        ...
        return postActivity.find(p => p._id === week)?.count || 0;
      });
      ...
      adminStats = {
        totals: { users: await db.collection('users').countDocuments(), activeUsers, deactiveUsers, books: totalBooks, posts: totalPosts, reports: totalReports },
        charts: { userGrowth: { labels: months, data: growthData }, postActivity: { labels: ['Week 1', 'Week 2', 'Week 3', 'Week 4'], data: activityData } },
        recentUsers
      };
    }
    ...
    if (['librarian', 'admin'].includes(user.role)) {
      ...
      const borrowTrend = await db.collection('borrows').aggregate([
        ...
        { $group: { _id: { $isoWeek: '$borrowedAt' }, count: { $sum: 1 } } },
        { $sort: { _id: 1 } }
      ]).toArray();
    ...
    if (user.role === 'student') {
      ...
      // 5. Reading progress – mock (replace later with real data)
      const readingProgress = Math.floor(Math.random() * 30) + 50; // 50–80%
