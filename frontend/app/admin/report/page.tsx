          ...
          <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
            <tr>
              {Object.keys(items[0]).map((k) => (
                <th key={k} className="px-4 py-3 text-left font-medium text-gray-700 capitalize">
                  {t(k.replace(/_/g, ' '))}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-gray-100">
            {items.map((row, i) => (
              <motion.tr
                ...
                className="hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 transition"
              >
                {Object.values(row).map((v: any, j) => (
                  <td key={j} className="px-4 py-3 text-gray-800 font-medium">
                    {v instanceof Date 
